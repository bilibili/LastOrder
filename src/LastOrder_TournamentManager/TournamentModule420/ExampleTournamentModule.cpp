...
public:
	...
	void update(std::vector<int> & times, int killCount)
	{
		int killParameter = 100;
		...
		if (BWAPI::Broodwar->self()->getName() == "LastOrder")
		{
			killScore = killScore - killCount * killParameter;
			killScore = killScore < 0 ? 0 : killScore;
		}
	...
	bool write(std::string filename, std::vector<int> allGameScore)
	{
		gameTimeUp = Broodwar->getFrameCount() > 85714;
		std::stringstream serializeString;
		for (size_t i = 0; i < allGameScore.size(); i++)
		{
			serializeString << allGameScore[i] << ",";
		}
		...
		if (outfile.is_open())
		{
			...
			outfile << serializeString.str() << std::endl;
...
{
	//BWAPI::Broodwar->enableFlag(Flag::CompleteMapInformation);
	...
	totalSunkenSporeKillCount = 0;
	nextRecordScoreTime = 0;
}


int ExampleTournamentAI::getAllDefenderKillCount()
{
	int killCount = 0;
	...
	return totalSunkenSporeKillCount + killCount;
}


void ExampleTournamentAI::onEnd(bool isWinner)
{
	int killCount = getAllDefenderKillCount();
	TournamentModuleState state = TournamentModuleState();
	state.ended();
	state.update(timerLimitsExceeded, killCount);
	state.write("gameState.txt", allGameScore);
}


void ExampleTournamentAI::onFrame()
{
	if (Broodwar->getFrameCount() > nextRecordScoreTime)
	{
		nextRecordScoreTime = Broodwar->getFrameCount() + 24 * 10;
		int killCount = getAllDefenderKillCount();
		TournamentModuleState state = TournamentModuleState();
		state.update(timerLimitsExceeded, killCount);
		allGameScore.push_back(state.selfScore);
	}
...
void ExampleTournamentAI::onUnitDestroy(BWAPI::Unit unit)
{
	...
	if (unit->getPlayer() == BWAPI::Broodwar->self() && 
		...
		unit->getType() == BWAPI::UnitTypes::Zerg_Spore_Colony))
	{
		totalSunkenSporeKillCount += unit->getKillCount();
	}
}
