...
public class Server  extends Thread 
{
	...
	private int							gameRescheduleTimer = 50;
	...
	public void run()
	{
		...
		while (true)
		{
			...
			{		
				...
				{
					//log("enter free.size() < neededClients \n");
					if (!notEnoughClients)
					{
				...
				Set<String> startingBots = getRunningHostBotNames();
				//log("current starting and running bots \n");
				//for (String b : startingBots){
				//	log(b + '\n');
				//}
				...
				{
					//log("enter ServerSettings.Instance().StartGamesSimul \n");
					// we can start multiple games at same time
					nextGame = games.getNextGame(startingBots, freeClientProperties, ServerSettings.Instance().EnableBotFileIO, gui);
					if (nextGame == null)
					{
						//log("no next games \n");
				...
				else if (startingBots.isEmpty())
				{
					...
					nextGame = games.getNextGame(null, freeClientProperties, ServerSettings.Instance().EnableBotFileIO, gui);
					if (nextGame == null)
					{
						//can't start a game because of bot requirements or round
						if (!requirementsNotMet)
				...
				if(ServerSettings.Instance().EnableBotFileIO) {
					//log("enter ServerSettings.Instance().EnableBotFileIO \n");
	...
	private synchronized Set<String> getRunningHostBotNames()
	{
		Set<String> hostNames = new HashSet<String>();
		for (int i = 0; i < clients.size(); i++)
		{
			...
			if (c.getStatus() == ClientStatus.STARTING || c.getStatus() == ClientStatus.SENDING )//|| c.getStatus() == ClientStatus.RUNNING )
			{
				hostNames.add(c.lastInstructionSent.hostBot.getName());
			}
		}
