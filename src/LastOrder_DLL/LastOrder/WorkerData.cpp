...
bool WorkerData::depotIsFull(BWAPI::Unit depot)
{
	...
	if (assignedWorkers >= int(mineralsNearDepot * 2.5) && assignedWorkers > 0)
	{
		return true;
	}
	else
...
std::vector<BWAPI::Unit> WorkerData::getMineralPatchesNearDepot(BWAPI::Unit depot)
{
	...
	const std::vector<const ChokePoint *> & depotChokes = BWEMMap.GetArea(depot->getTilePosition())->ChokePoints();
	std::set<BWAPI::Unit> mineralsAtChoke;
	for (auto const& m : BWAPI::Broodwar->getMinerals())
	{
		for (auto const& r : depotChokes)
		{
			if (m->getInitialPosition().getDistance(Position(r->Center())) < 2 * 32)
			{
...
void WorkerData::balanceWorker(BWAPI::Unit depot)
{
	...
	for(auto const& count : depotWorkerCount)
	{
...
BWAPI::Unit WorkerData::getMineralToMine(BWAPI::Unit worker)
{
	...
	if (depot)
	{
		...
		for(auto const& mineral : mineralPatches)
		{
